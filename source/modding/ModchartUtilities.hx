...
class ModchartUtilities
{
    ...
    function new(?path:Null<String>)
    {
        ...
        if (result != 0)
        {
            CoolUtil.coolError("lua COMPILE ERROR:\n" + Lua.tostring(lua,result),"JP Support + Leather Engine Modcharts");
        ...
        Lua_helper.add_callback(lua,"makeStageSprite", function(id:String, filename:String, x:Float, y:Float, size:Float = 1) {
            if(!lua_Sprites.exists(id))
            {
                ...
                CoolUtil.coolError("Sprite " + id + " already exists! Choose a different name!", "JP Support + Leather Engine Modcharts");
        });
