...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		kotoba = utilities.Options.getData("kotoba");
		...
		if (!yoWaitThisIsCharter)
		{
			...
			camPos.set(midPos.x + 150 + boyfriend.cameraOffset[0], midPos.y - 100 + boyfriend.cameraOffset[1]);
			...
			switch (funnyTimeBarStyle.toLowerCase())
			{
				...
				case "old kade engine":
						infoTxt.setFormat(Paths.font("vcr.ttf"), infoTxtSize, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				case "jpsych engine":
						infoTxt.setFormat(Paths.font("jpvcr.ttf"), infoTxtSize, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			}
	...
	public function updateScoreText()
	{
		...
		{
			default: // includes 'leather engine'
				scoreTxt.text = ("Score: " + songScore + " | " + "Misses: " + misses + " | " + "Health: " + (Math.floor(healthShown * 500)) / 10 + "%" + " | " + "Accuracy: " + accuracy + "% | " + ratingStr);
			case "jp":
				scoreTxt.text = ("スコア: " + songScore + " | " + "ミス数: " + misses + " | " + "体力: " + (Math.floor(healthShown * 500)) / 10 + "%" + " | " + "精度: " + accuracy + "% | " + ratingStr);
		}
	...
	public function returnStupidRatingText():String
	{
		...
		return ((utilities.Options.getData("marvelousRatings") ? "Combos: " + combo + "\n" : "")
			+ "Marvelous: "
	...
	function eventCharacterShit(event:Array<Dynamic>)
	{
		...
		if (gfMap.exists(event[3]) || bfMap.exists(event[3]) || dadMap.exists(event[3])) // prevent game crash
		{
			...
			CoolUtil.coolError("The character " + event[3] + " isn't in any character cache!\nHow did this happen? ¯|_(ツ)_|¯",
				"JP Support + Leather Engine's No Crash, We Help Fix Stuff Tool");
	...
	public function updateSongInfoText()
	{
		...
		switch (funnyTimeBarStyle.toLowerCase())
		{
			...
			case "jpsych engine":
				infoTxt.text = '${FlxStringUtil.formatTime(seconds, false)}'
					+ (utilities.Options.getData("botplay") ? " (ボット)" : "")
					+ (utilities.Options.getData("noDeath") ? " (死なない)" : "")
					+ (playingReplay ? " (リプレイ)" : "");
				infoTxt.screenCenter(X);
			case "old kade engine":
				infoTxt.text = SONG.song
